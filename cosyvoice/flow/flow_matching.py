...
class ConditionalCFM(BASECFM):
    ...
    def compute_loss(self, x1, mask, mu, spks=None, cond=None, streaming=False):
        ...
        # the estimator (DiT) may trim its internal sequence length by a frame or two to
        # reconcile token-rate vs audio-frame-rate lengths (see dit.py's DiT.forward comment)
        # -- mask/u here are untouched by that, so match pred's actual length before using them.
        min_len = min(pred.shape[2], mask.shape[-1], u.shape[2])
        pred, mask, u = pred[:, :, :min_len], mask[..., :min_len], u[:, :, :min_len]
