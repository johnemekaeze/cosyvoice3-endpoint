...
class DiT(nn.Module):
    ...
    def forward(self, x, mask, mu, t, spks=None, cond=None, streaming=False):
        ...
        # mu comes from the token-rate encoder output (upsampled via token_mel_ratio) while
        # x/cond/mask come from the real audio's fbank frame count -- with num_frames not
        # wired through compute_fbank's binding in this checkout (see processor.py), these
        # can differ by a frame or two. Trim everything to the shortest common length rather
        # than crash: losing a frame off ~80 doesn't meaningfully affect training.
        min_len = min(x.shape[1], mu.shape[1], cond.shape[1], mask.shape[-1])
        x, mu, cond = x[:, :min_len], mu[:, :min_len], cond[:, :min_len]
        mask = mask[..., :min_len]
