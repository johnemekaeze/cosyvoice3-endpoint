...
def compute_fbank(data,
                  feat_extractor,
                  num_frames=-1,
                  token_mel_ratio=None,
                  mode='train'):
    # token_mel_ratio: cosyvoice/dataset/dataset.py's Dataset() binds this kwarg for GAN
    # training (token_mel_ratio=0) but this processor.py doesn't otherwise use it -- the
    # GAN pipeline's `truncate` stage (truncate_length, a multiple of hop_size) already runs
    # before compute_fbank and fixes sample['speech'] to an exact length, and num_frames
    # (this function's own alignment knob) isn't set by that same binding, so there is
    # nothing left for this function to do with the ratio; accepted only so the call doesn't
    # raise TypeError (unconditional upstream import/wiring, no fix available upstream on
    # the currently-checked-out commit -- see cosyvoice/bin/train.py's deepspeed comment for
    # the same class of gap between dataset.py and this file).
    """ Extract fbank
