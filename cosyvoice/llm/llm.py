...
# <|endofprompt|> token id, as assigned by CosyVoice3Tokenizer's add_special_tokens() call at
# construction time (cosyvoice/tokenizer/tokenizer.py) -- it's appended right after whatever
# the loaded tokenizer's base vocab length is, so this value tracks that base vocab size, not
# a fixed Qwen constant. Stock CosyVoice3-BlankEN's base vocab was 151646 (hence the original
# "151646 is <|endofprompt|>" comments below); this project's tokenizer had 271 African-language
# characters added directly to the base vocab (add_tokens, not add_special_tokens) across
# 2026-08-22, which pushed <|endofprompt|>'s runtime position to 151920 -- reconfirm by encoding
# "<|endofprompt|>" through the live CosyVoice3Tokenizer if the base vocab size changes again.
EOP_TOKEN_ID = 151920
...
class Qwen2LM(TransformerLM):
    ...
    def inference(
        ...
        if self.__class__.__name__ == 'CosyVoice3LM':
            # NOTE temporary hardcode -- was 151646 for the stock base vocab; our tokenizer's
            # base vocab grew (African-language tokens added directly via add_tokens, not
            # through add_special_tokens), which shifts where CosyVoice3Tokenizer's own
            # add_special_tokens() call appends <|endofprompt|> at construction time. Re-check
            # this value (encode "<|endofprompt|>" through the live tokenizer) if the vocab
            # size changes again.
            assert EOP_TOKEN_ID in text, '<|endofprompt|> not detected in CosyVoice3 text or prompt_text, check your input!'
    ...
    def inference_bistream(
        ...
        if self.__class__.__name__ == 'CosyVoice3LM':
            # NOTE temporary hardcode -- see the matching comment above; re-check if vocab size changes.
            assert EOP_TOKEN_ID in prompt_text, '<|endofprompt|> not detected in CosyVoice3 prompt_text, check your input!'
            eop_index = prompt_text.flatten().tolist().index(EOP_TOKEN_ID)
