...
# deepspeed is only needed for --train_engine deepspeed (all uses below are already gated on
# that). Import lazily: this env's deepspeed release needs torch.library.custom_op (PyTorch
# 2.4+), which torch 2.3.1 here doesn't have -- an unconditional top-level import would break
# torch_ddp runs that never touch deepspeed at all. See cosyvoice/bin/train.py for the same fix.
try:
    import deepspeed
except Exception:
    deepspeed = None
# wandb is optional and only touched when WANDB_PROJECT is set (see init_summarywriter) --
# lazy-import so a run without it configured never depends on the package or its network calls.
try:
    import wandb
except Exception:
    wandb = None
...
def init_summarywriter(args):
    ...
    if int(os.environ.get('RANK', 0)) == 0:
        ...
        # Opt-in: only touches the network/wandb package when WANDB_PROJECT is set, so runs
        # that don't configure it behave exactly as before. Defaults to offline since compute
        # nodes on this cluster aren't guaranteed outbound internet -- `wandb sync` the run
        # directory from a login node afterward, or export WANDB_MODE=online to skip that.
        if wandb is not None and os.environ.get('WANDB_PROJECT'):
            os.environ.setdefault('WANDB_MODE', 'offline')
            wandb.init(
                project=os.environ['WANDB_PROJECT'],
                name=os.environ.get('WANDB_RUN_NAME', os.path.basename(args.model_dir.rstrip('/'))),
                dir=args.model_dir,
                config=vars(args),
                resume='allow',
            )
        ...
        if (info_dict['train_engine'] == 'deepspeed' and info_dict['is_gradient_accumulation_boundary'] is True) or \
           (info_dict['train_engine'] == 'torch_ddp' and (info_dict['batch_idx'] + 1) % info_dict['accum_grad'] == 0):
            ...
            if wandb is not None and wandb.run is not None:
                wandb.log({'{}/{}'.format(tag, k): info_dict[k] for k in ['epoch', 'lr', 'grad_norm']} |
                           {'{}/{}'.format(tag, k): v for k, v in loss_dict.items()}, step=step + 1)
...
def log_per_save(writer, info_dict):
    ...
    if writer is not None:
        ...
        for k, v in loss_dict.items():
            writer.add_scalar('{}/{}'.format(tag, k), v, step + 1)
        if wandb is not None and wandb.run is not None:
            wandb.log({'{}/{}'.format(tag, k): info_dict[k] for k in ['epoch', 'lr']} |
                       {'{}/{}'.format(tag, k): v for k, v in loss_dict.items()}, step=step + 1)
